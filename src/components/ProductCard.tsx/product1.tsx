...
import prod from "../data/products1.json";
import { Box, Container } from "@mui/material";
import { store } from "../redux/store";
import { setCartProducts, setCartProducts1 } from "../slices/productsSlice";
...
export default function Products() {
  ...
  const addCartItems = (e: any) => {
    const addedProduct = prod.products1.find((item: any) => item.title === e);
    if (addedProduct) {
      prodArr = [...prodArr, addedProduct];
    }
    store.dispatch(setCartProducts1(prodArr));
  };
  return (
    <>
      <Box>
        {prod.products1.map((item, index) => (
